...
class TemplatesModule(Module):
    """Adds a processor to all files which runs them through templates. The
    templates should be present in the templates directory which is specified
    in the config file. The path to the template directory is a path relative
    to the source directory root. The template system will try to match the name
    of the template file to the name of the output file in the following manner:
