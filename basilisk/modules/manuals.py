...
class ManualsModule(Module):
    """Injects a processor which converts troff files to plaintext. This is
    performed on files which are in the directories defined in the config file.
    The keys of the dictionary "manuals" defined in the config file should
    contain the names of troff macros which will be used to process the files.
    The value of each key should be a list of directories. All files present in
    those directories will be converted.
